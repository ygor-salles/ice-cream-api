...
describe('Clients', () => {
  ...
  it('Should returns 400 beacause there is no valid client debit', async () => {
    ...
    expect(response.body.message).toBe(
      'debit must be a `number` type, but the final value was: `NaN` (cast from the value `"aaa"`).',
    );
  });
  ...
  it('Should return 400 when searching client by invalid type id', async () => {
    const response = await request(app).get(`/clients/aaa`).set('Authorization', `bearer ${token}`);
